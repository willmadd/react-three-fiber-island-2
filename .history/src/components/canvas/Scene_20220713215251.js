...
const Scene = (props) => {
  ...
  const cameraPosition = useMemo(() => {
    return [boatRotation[0], boatRotation[1] + 100, boatRotation[2] + 1200]
  }, [cameraPosition])
  ...
  useFrame((state, delta) => {
    if (moveLeft) {
      fishingBoatRef.current.rotation.y += 0.011
    } else if (moveRight) {
      fishingBoatRef.current.rotation.y -= 0.01
    }
    if (moveForward) {
      console.log('move forward')
      // fishingBoatRef.current.speed = 1
      console.log(fishingBoatRef.current.speed)
      // fishingBoatRef.current.rotation.y += 1
      fishingBoatRef.current.translateX(1)
      // setBoatPosition((prev) => [prev[0], prev[1] + 0.1, prev[2]])
    }
    // else if (moveForward) {
    // }
  })
